...
if ($res->is_success) {
	...
	# next token request
	if ($response_body =~ /name="frmDefender"/ || $response_body =~ /name="frmNextToken"/ || $response_body =~ /Additional Credentials Page/){
    ...
    #if ( $response_body =~ /Additional Credentials Page/){
    #    print "Additional Credentials Page\n";
	#	my $token=read_password();
	#	$res = $ua->post($res->base(),
	#		[ btnSubmit   => 'Sign In',
	#		'password#2' => $token,
	#		]);
	#	$response_body=$res->decoded_content;
	...
	if ($response_body =~ /id="DSIDConfirmForm"/) {
		print "Atleast one active sessions found, choose wich to terminate:\n";
        my $itr = 0;
        my @sessions = ();
        while ($response_body =~ m/id="(postfixSID_\d+)".*value="(\w+)".*<td>([\d\.]+)<\/td>\s+<td>([^<>]+)<\/td>\s+<td>([^<>]+)<\/td>\s+<td>([^<>]+)<\/td>.*<\/tr>/gs) {
                print "$itr: $3 $4 $5 $6\n";
                $sessions[$itr] = $2;
        }
        my $userinput =  <STDIN>;
        chomp ($userinput);
        if( exists $sessions[$userinput] ) {
            $response_body =~ m/name="FormDataStr" value="([^"]+)"/;
                    my $formDataStr=$1;

        $res = $ua->post("https://$dhost:$dport/dana-na/auth/$durl/login.cgi",
            [ 'btnContinue'  => 'Close Selected Sessions and Log in',
            'postfixSID'  => $sessions[$userinput],
